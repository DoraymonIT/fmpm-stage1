                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <form action="soutenance_process.php" id="my_form" method="post">
                            <h5 class="crenau"><i class="fa fa-info-circle" aria-hidden="true"></i> Informations
                                Principales
                            </h5>
                            ...
                            <?php //echo $soutenance['date_depot_sujet']
                            ?><!--</span>--><input type="hidden" name="soutenance_id"
                                                   value="<?php echo $soutenance['soutenance_id'] ?>"/>
                            <input class="datepicker1 form-control form-control-sm" type="date"
                                   placeholder='&#128197; Cliquez Ici pour Choisir un jour . &#x1f4c5;'
                                   style="text-align: center;" name="date_depot_sujet"
                                   value="<?php echo $soutenance['date_depot_sujet'] ?>"
                                   disabled required/>
                            <br/>
                            <label>Directeur de la thèse <span>*</span></label>
                            <select class="form-control form-control-sm" name="directeur_these"
                                    onclick="profSelect(this)" <?php if ($soutenance['etat'] < -2) echo "disabled"; ?>
                                    required>
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    ?>
                                    <option <?php if ($soutenance['directeur'] == $row['id']) echo "selected"; ?>
                            ...
                            <input type="text" name="intitule" class="form-control form-control-sm"
                                   placeholder="Intitulé de la thèse"
                                   value="<?php echo $soutenance['intitule_these'] ?>" required/>
                            <br/>
                            <label>Nature de la thèse <span>*</span></label>
                            <input type="text" name="nature" class="form-control form-control-sm"
                                   placeholder="Nature de la thèse" value="<?php echo $soutenance['nature_these'] ?>"
                                   required/>
                            <br/>
                            ...
                            <input type="text" name="materiel_echan" class="form-control form-control-sm"
                                   placeholder="Matériel d’étude et échantillonnage"
                                   value="<?php echo $soutenance['materiel_d_etude_et_echantillioannage'] ?>" required/>
                            <br/>
                            <label> Durée de l’étude<span>*</span></label>
                            <input type="text" name="duree" class="form-control form-control-sm"
                                   placeholder="Durée de l’étude"
                                   value="<?php echo $soutenance['duree_d_etude'] ?>"
                                   required/>
                            <br/>
                            <label>Lieu de l’étude <span>*</span></label>
                            <input type="text" name="lieu" class="form-control form-control-sm"
                                   placeholder="Lieu de l’étude "
                                   value="<?php echo $soutenance['lieu_d_etude'] ?>"
                                   required/>
                            <br/>
                            <label>Objectifs de l’étude (ne pas dépasser 2 lignes)
                                <span>*</span></label>
                            <textarea name="obj_etude" id="" cols="2" rows="2" class="form-control"
                                      placeholder="Objectifs de l’étude"
                                      required><?php echo $soutenance['objectif_d_etude'] ?></textarea>
                            <br/>
                            <label> Mots clés <span>*</span></label>
                            <input type="text" name="mots_cles" class="form-control form-control-sm"
                                   placeholder="Mots clés"
                                   value="<?php echo $soutenance['mots_cles'] ?>" required/>
                            <small id="mot" class="form-text text-muted">Utilisez virgule <b>( , )</b> entre un mot et
                                autre
                                .</small>
                            ...
                            <select class="form-control form-control-sm" name="president_these"
                                    onclick="profSelect(this)" <?php if ($soutenance['etat'] < -3) echo "disabled"; ?>
                                    required>
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    ?>
                                    <option <?php if ($soutenance['president'] == $row['id']) echo "selected"; ?>
                                            value="<?php echo $row['id'] ?>"><?php echo $row['nom'] . " " . $row['prenom']; ?></option>
                                <?php } ?>
                            </select>
                            <br/>
                            <label>Membre 1 du Jury <span>*</span></label>
                            <select class="form-control form-control-sm" name="jury1" onclick="profSelect(this)"
                                    required>
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    ?>
                                    <option <?php if ($soutenance['jury1'] == $row['id']) echo "selected"; ?>
                                            value="<?php echo $row['id'] ?>"><?php echo $row['nom'] . " " . $row['prenom']; ?></option>
                                <?php } ?>
                            </select>
                            <br/>
                            <label>Membre 2 du Jury <span>*</span></label>
                            <select class="form-control form-control-sm" name="jury2" onclick="profSelect(this)"
                                    required>
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    ?>
                                    <option <?php if ($soutenance['jury2'] == $row['id']) echo "selected"; ?>
                                            value="<?php echo $row['id'] ?>"><?php echo $row['nom'] . " " . $row['prenom']; ?></option>
                                <?php } ?>
                            </select>
                            <br/>
                            <label>Membre 3 du Jury<span>*</span></label>
                            <select class="form-control form-control-sm" name="jury3" onclick="profSelect(this)"
                                    required>
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    ?>
                                    <option <?php if ($soutenance['jury3'] == $row['id']) echo "selected"; ?>
                                            value="<?php echo $row['id'] ?>"><?php echo $row['nom'] . " " . $row['prenom']; ?></option>
                                <?php } ?>
                            </select>
                            <br/>
                            <label>Membre 4 du Jury <span>*</span></label>
                            <select class="form-control form-control-sm" name="jury4" onclick="profSelect(this)"
                                    required>
                            ...
                            <div class="custom-control custom-switch">
                                <input type="checkbox" name="creneau_change" class="custom-control-input"
                                       id="customSwitches">
                                <label class="custom-control-label" for="customSwitches">Changer votre creneau choisi
                                    ?</label>
                            </div>
                            ...
                            <hr>
                            <br/>
                            <button type="submit" name="submit_edit_soutenance" class="btn btn-success btn-custom"><i
