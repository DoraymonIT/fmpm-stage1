...
--
-- Hôte : 127.0.0.1
-- Généré le : Dim 07 mars 2021 à 21:13
...
INSERT INTO `soutenance` (`soutenance_id`, `directeur`, `president`, `jury1`, `jury2`, `jury3`, `jury4`, `etudiant`, `these`, `creneau`, `etat`, `motif`) VALUES
(2, 1, 2, 4, 5, 6, 7, 'G145625311', 4, NULL, 0, NULL);
...
INSERT INTO `these` (`id`, `intitule`, `directeur`, `nature_etude`, `materiel_etude_echan`, `duree_etude`, `lieu_etude`, `objectifes_etude`, `mots_cles`, `date_depot`, `etudiant`, `etat`, `motif`) VALUES
(4, 'sddsda', 1, 'adadada', 'dddd', 'adasdasdasda', 'adadsadsd', 'sss', 'sssss', '0000-00-00', 'G145625311', 0, '');
...
ALTER TABLE `these`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_prof_these` (`directeur`),
  ADD KEY `fk_foreign_these_etudiant` (`etudiant`);
...
--
ALTER TABLE `soutenance`
  ADD CONSTRAINT `fk_foreign_soutenance_these` FOREIGN KEY (`these`) REFERENCES `these` (`id`),
  ...
  ADD CONSTRAINT `soutenance_ibfk_9` FOREIGN KEY (`directeur`) REFERENCES `prof` (`id`);

--
-- Contraintes pour la table `these`
--
ALTER TABLE `these`
  ADD CONSTRAINT `fk_foreign_these_etudiant` FOREIGN KEY (`etudiant`) REFERENCES `etudiant` (`CNE`),
  ADD CONSTRAINT `id_prof_these` FOREIGN KEY (`directeur`) REFERENCES `prof` (`id`);
